...
function BindMenu() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var MainMenu = "";
            var Qualification = "<ul class='dropdown'>";
            var CatgorySaubject = "<ul class='dropdown'>";


            for (var i = 0; i < obj.Table1.length; i++) { // for loop start for Bind Qualification

                var ExamBoard = "<ul class='dropdown'>"; // "<ul>";

                for (var j = 0; j < obj.Table2.length; j++) { // for loop start for Bind ExamBoard

                    var Subject = "  <ul class='dropdown'>"; //"<ul>";
                    for (var l = 0; l < obj.Table3.length; l++) {// for loop start for Bind Subject

                        if (obj.Table2[j].GQEMID == obj.Table3[l].GQEMID) {
                            Subject += "<li><a  id='SubjectlnkMenu-" + obj.Table3[l].SubjectID + "-Unit-" + obj.Table3[l].SubjectName + "' href='../NewPages/ExamBoardInfo.aspx?SubjectID=" + obj.Table3[l].SubjectID + "&SubjectNames=" + obj.Table3[l].SubjectName + "'>" + obj.Table3[l].SubjectName + "</a></li>";
            ...
            for (var k = 0; k < obj.Table.length; k++) {
                ...
                if (obj.Table[k].MenuId == '1') {
                    ...
                    MainMenu += " <li ><a  id='AllQlnkMenu-" + obj.Table[k].MenuId + "-Qualification-" + obj.Table1[0].Qualification + "'   href='../NewPages/ExamBoardInfo.aspx?Type=Qualification'>" + obj.Table[k].MenuName + "</a>" + Qualification + "</li>";
                    ...
                    MainMenu += " <li ><a  id='SubjectforCategory-" + obj.Table[k].MenuId + "-Category-Biology'   href='../NewPages/ExamBoardInfo.aspx?Type=SubjectCategory'>" + obj.Table[k].MenuName + "</a>" + CatgorySaubject + "</li>";
                }
                else {
