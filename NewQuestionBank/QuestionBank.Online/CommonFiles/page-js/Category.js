...
function BindQualification(Qualification) {
    var InputXMLParam = "";
    ...
    $.ajax({
        ...
        success: function (data) {
            var respCode = data.d.responseCode;
            $('#Qualification').html('');
            $('#Qualification').append($("<option></option>").val(0).html('--Select--'));

            if (respCode == 0) {
                obj = jQuery.parseJSON(data.d.responseData);
                for (var i = 0; i < obj.Table1.length; i++) {
                    $('#Qualification').append($("<option></option>").val(obj.Table1[i].Qualification).html(obj.Table1[i].Qualification));
                }
                if (Qualification != '')
                    $("#Qualification option:contains('" + Qualification + "')").attr('selected', 'selected');
...
function EditCategory(btn) {
    ...
    $("#SubjectName option:contains('" + $(btn).parent().parent().find("td")[2].innerHTML + "')").attr('selected', 'selected');
   //$("#Qualification option:contains('" + $(btn).parent().parent().find("td")[3].innerHTML + "')").attr('selected', 'selected');

    $("#txtCategoryName").val($(btn).parent().parent().find("td")[4].innerHTML)

    if ($(btn).parent().parent().find("td")[5].innerHTML == "true")
        $("#chkIsActive").prop("checked", true);
    else
        $("#chkIsActive").prop("checked", false);

    $("#Qualification").val($(btn).parent().parent().find("td")[3].innerHTML);
    $('#myModal').modal('toggle');
    $('#myModal').modal('show');
}

function SaveCategory() {
    var InputXMLParam = "<Root><CategoryID>" + $("#CategoryID").val() + "</CategoryID><CategoryName>" + $("#txtCategoryName").val() + "</CategoryName><SubjectID>" + $("#SubjectName").val() + "</SubjectID><Qualification>" + $("#Qualification").val() + "</Qualification><IsActive>" + $("#chkIsActive").prop("checked") + "</IsActive></Root>";
